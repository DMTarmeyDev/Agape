# Agape Core V3.1 Early Alpha R8

R8 keeps the guided flow and lets Step 2 accept either pasted text or a chosen document. Supported document import includes TXT, Markdown, JSON and DOCX plus common source/text formats. Imported text is loaded into the same editable box before saving.
...
## V3.1 Early Alpha R3 installation-evidence repair

R3 preserves Chat Auto Choose from R2 and repairs the installation preflight contract. The one-file updater runs the real AI-to-terminal, Template 1, Template 2 and bounded Project Loop preflights against an isolated temporary data root, writes `install-receipt.json` only after every required preflight passes, then verifies the live `/api/system-test` reaches 14/14. Failed preflight does not mint a PASS receipt and triggers source rollback.


## V3.1 R4
Chat Auto Choose is now a project-aware button. It requires a loaded project and evaluates project metadata, saved goal/workspace, codebase size/languages, recent project conversation, open issues, current task, and prior Project Loop outcomes before selecting an installed model. Manual model selection remains available.
