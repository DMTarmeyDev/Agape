...
# Agape Mainframe writing-quality checker. Document Studio still runs if the
# shared module is unavailable, but bundled Mainframe builds use it for every
# pasted/uploaded source document.
try:
    _MAINFRAME_ROOT = Path(__file__).resolve().parents[2]
    if str(_MAINFRAME_ROOT) not in sys.path:
        sys.path.insert(0, str(_MAINFRAME_ROOT))
    from agape_mainframe.writing_quality import proofread_text as _agape_proofread_text
except Exception:
    _agape_proofread_text = None
...
def _configured_path(env_name, config_name):
    override=str(os.environ.get(env_name) or "").strip()
    if override:
        return Path(override).expanduser().resolve()
    raw=Path(str(CONFIG[config_name])).expanduser()
    return raw.resolve() if raw.is_absolute() else (ROOT / raw).resolve()

DATA = _configured_path("AGAPE_DOCUMENT_DATA", "data_root")
TEMPLATES = _configured_path("AGAPE_DOCUMENT_TEMPLATES", "template_library")
GENERATED = TEMPLATES / "generated-open-format"
EXTERNAL = TEMPLATES / "external-open-source"
OUTPUTS = _configured_path("AGAPE_DOCUMENT_OUTPUTS", "output_root")
CACHE = DATA / "open-source-cache"
HISTORY_DB = DATA / "document-history.sqlite3"
SOURCE_REPORT = DATA / "template-source-status.json"
SETTINGS_PATH = Path(os.environ.get("AGAPE_DOCUMENT_SETTINGS") or (ROOT / "settings.json")).expanduser().resolve()
...
def find_soffice():
    configured=str(CONFIG.get("soffice") or "").strip()
    cands=[]
    if configured and configured.lower() != "auto":
        cands.append(configured)

    if os.name == "nt":
        # On Windows, background automation must use soffice.exe. The generic
        # launcher can resolve to soffice.com and create an unwanted console.
        cands += [
        ...
        # Linux packages normally expose `soffice` (sometimes `libreoffice`).
        # The macOS app bundle includes its executable at this standard path.
        cands += [
    ...
    for x in cands:
        if not x:
            continue
        try:
            p=Path(str(x).strip('"')).expanduser().resolve()
            key=str(p).casefold()
            if key in seen:
                continue
            seen.add(key)
            if os.name == "nt":
                if p.name.lower() != "soffice.exe" or p.suffix.lower() != ".exe":
                    continue
            elif p.name.lower() not in {"soffice", "libreoffice"}:
                continue
            if p.exists() and p.is_file() and (os.name == "nt" or os.access(p, os.X_OK)):
                return str(p)
        except Exception:
            pass
    return None
...
def libreoffice_detail():
    s=find_soffice()
    if not s:
        return {"installed":False,"path":None,"version":None,"error":"LibreOffice soffice executable not found"}
    try:
        p=Path(s)
        stat=p.stat()
...
AI_PROVIDER_MODELS={
    ...
    "ollama":os.environ.get("AGAPE_OLLAMA_DOCUMENT_MODEL","qwen2.5-coder:1.5b-instruct"),
}
...
def _run_status_command(provider,force=False):
    ...
    if held and not force:
        held_exe=str(held.get("executable") or "")
        if held_exe and not _cli_executable_valid(held_exe):
            _clear_auth_hold(provider);_AUTH_CACHE.pop(provider,None);held=None
        else:
            return {"provider":provider,"installed":True,"logged_in":True,"executable":held_exe,"status":"CONNECTED_HELD","held_for_session":True,"expires_at":held.get("expires_at"),"detail":"Authenticated provider held for the active Agape session."}
...
def _call_ollama(prompt,system_prompt,max_tokens,timeout,requested_model=None):
    ...
    prefs=[requested_model,AI_PROVIDER_MODELS[provider],"qwen2.5-coder:1.5b-instruct","qwen2.5-coder:7b"]
    ...
    try:
        local_tokens=max(128,min(int(max_tokens or 0),1400))
        local_timeout=max(45,min(int(timeout or 240),240))
        r=requests.post("http://127.0.0.1:11434/api/chat",json={"model":model,"messages":_openai_messages(prompt,system_prompt),"stream":False,"options":{"temperature":0.15,"num_predict":local_tokens}},timeout=local_timeout)
    except requests.Timeout as e:raise AdaptiveAIError(provider,"transient",408,str(e))
    except requests.RequestException as e:raise AdaptiveAIError(provider,"transient",None,str(e))
...
AI_PREFLIGHT_CACHE={}
AI_PREFLIGHT_TTL=300

def _provider_runtime_preflight(provider, task="general", force=False):
    provider=str(provider or "").lower().strip();now=time.time()
    cached=AI_PREFLIGHT_CACHE.get(provider)
    if cached and not force and now-float(cached.get("at") or 0)<AI_PREFLIGHT_TTL:
        return dict(cached.get("value") or {})
    result={"ok":False,"provider":provider,"reason":"provider_preflight_failed","detail":"Provider is not ready"}
    try:
        if provider in ("chatgpt","claude"):
            mode=_provider_auth_mode(provider)
            st=_run_status_command(provider,True)
            if st.get("logged_in") and mode in ("auto","account"):
                exe=str(st.get("executable") or "")
                if not _cli_executable_valid(exe):
                    raise RuntimeError("subscription CLI path is missing: "+exe)
                BRAIN_WORKDIR.mkdir(parents=True,exist_ok=True)
                # Verify the actual execution path once per short cache window; login-status alone
                # cannot catch stale/moved CLI executables or an unusable working directory.
                if provider=="chatgpt":
                    out=BRAIN_WORKDIR/("codex-preflight-"+hashlib.sha1(os.urandom(12)).hexdigest()[:8]+".txt")
                    try:
                        proc=_run_cli_utf8(_codex_exec_args(exe,out),input="Reply exactly: AGAPE_PROVIDER_OK",capture_output=True,timeout=35,env=_clean_cli_env(provider),cwd=str(BRAIN_WORKDIR),**hidden_process_kwargs())
                        stream=_parse_codex_jsonl(proc.stdout or "")
                        text=(out.read_text(encoding="utf-8",errors="replace").strip() if out.exists() else "") or str(stream.get("final_message") or "").strip()
                        if not stream.get("turn_completed") and not text:
                            raise RuntimeError(str(stream.get("fatal_error") or proc.stderr or "Codex execution preflight did not complete")[:500])
                    finally:
                        try:out.unlink(missing_ok=True)
                        except Exception:pass
                else:
                    proc=_run_cli_utf8([exe,"--restricted","-p","Reply exactly: AGAPE_PROVIDER_OK","--no-session-persistence","--permission-prompts","none","--max-turns","1","--output-format","text"],capture_output=True,timeout=35,env=_clean_cli_env(provider),cwd=str(BRAIN_WORKDIR),**hidden_process_kwargs())
                    if proc.returncode!=0 or not str(proc.stdout or "").strip():
                        raise RuntimeError(str(proc.stderr or proc.stdout or "Claude execution preflight failed")[:500])
                result={"ok":True,"provider":provider,"mode":"account","detail":"Subscription execution path verified"}
            else:
                if mode=="account":
                    raise RuntimeError("account login is not ready")
                keytest=_test_provider_key(provider)
                if not keytest.get("ok"):
                    raise RuntimeError(str(keytest.get("error") or "API key test failed"))
                result={"ok":True,"provider":provider,"mode":"api_key","detail":"API key verified"}
        elif provider=="ollama":
            models=_ollama_models();preferred=AI_PROVIDER_MODELS.get("ollama")
            model=preferred if preferred in models else ("qwen2.5-coder:1.5b-instruct" if "qwen2.5-coder:1.5b-instruct" in models else (models[0] if models else ""))
            ready=ensure_ollama_engine(model,True,wait_seconds=12)
            if not ready.get("ok"):
                raise RuntimeError(str(ready.get("error") or "Ollama/model not ready"))
            probe=requests.post("http://127.0.0.1:11434/api/generate",json={"model":model,"prompt":"Reply OK","stream":False,"options":{"temperature":0,"num_predict":1}},timeout=(5,45))
            if not probe.ok:
                raise RuntimeError("Ollama preflight HTTP "+str(probe.status_code))
            result={"ok":True,"provider":provider,"mode":"local","model":model,"detail":"Ollama generated a live probe response"}
        else:
            test=_test_provider_key(provider) if provider in ONLINE_PROVIDER_CATALOG else {"ok":_provider_credentials(provider)}
            if not test.get("ok"):
                raise RuntimeError(str(test.get("error") or "provider credential test failed"))
            result={"ok":True,"provider":provider,"mode":"api_key","detail":"Provider credential endpoint verified"}
    except Exception as exc:
        result={"ok":False,"provider":provider,"reason":"provider_preflight_failed","detail":str(exc)[:700]}
    AI_PREFLIGHT_CACHE[provider]={"at":now,"value":dict(result)}
    return result
...
def ai_generate(prompt,system_prompt=None,max_tokens=1800,requested_model=None,timeout=240,task=None,strict_provider=False):
    ...
    for provider in order:
        ...
        preflight=_provider_runtime_preflight(provider,task)
        if not preflight.get("ok"):
            attempts.append({"provider":provider,"status":"FAIL","phase":"preflight","reason":"provider_preflight_failed","detail":str(preflight.get("detail") or "Provider is not ready")[:260],"ms":int((time.perf_counter()-started)*1000)})
            AI_RUN_DISABLED[provider]="provider_preflight_failed"
            continue
        try:
    ...
    AI_LAST_ATTEMPTS=list(attempts)
    if attempts and not actual_attempted:
        raise RuntimeError("NO_READY_AI_PROVIDER: "+json.dumps(attempts,ensure_ascii=False))
    raise RuntimeError("ALL_SUITABLE_AI_PROVIDERS_FAILED: "+json.dumps(attempts,ensure_ascii=False))
...
def store_instruction_upload(d):
    ...
    extracted=ing.get("text") or ""
    source_kind=_instruction_source_kind(name,extracted)
    writing_check={"ok":True,"checker":"unavailable","chars_checked":len(extracted),"issue_count":0,"safe_corrections_applied":0}
    cleaned=extracted
    if _agape_proofread_text is not None and extracted:
        try:
            cleaned,writing_check=_agape_proofread_text(extracted)
        except Exception as exc:
            writing_check={"ok":False,"checker":"agape-writing-quality-v1","chars_checked":len(extracted),"issue_count":0,"safe_corrections_applied":0,"error":str(exc)}
            cleaned=extracted
    # The original file is preserved unchanged on disk. The AI-facing extracted
    # text receives only conservative mechanical corrections. Rebuild direct
    # chunks so RAG sees the same cleaned text. Specialist ingestion chunks are
    # kept as generated by their own parser.
    chunks=ing.get("chunks") or []
    if cleaned!=extracted and engine=="direct":
        chunks=_chunk_text_direct(cleaned)
    record={"id":upload_id,"name":name,"path":str(target),"ext":ext,"size":len(raw),"sha256":digest,"text":cleaned,"chars":len(cleaned),"source_kind":source_kind,"uploaded_at":datetime.now().isoformat(timespec="seconds"),"ingestion_engine":engine,"chunk_count":len(chunks) if chunks else int(ing.get("chunk_count") or 0),"chunks":chunks,"ingestion_fallback":ing.get("fallback") or "","writing_check":writing_check}
    ...
    return {k:record.get(k) for k in keys} | {"preview":cleaned[:12000],"full_text_available":True,"ai_stored_chars":len(cleaned),"writing_check":writing_check}
...
def _review_ready_pool(requested):
    requested=list(dict.fromkeys(str(x or "").lower().strip() for x in (requested or []) if str(x or "").strip()))[:10]
    ready=[];errors=[]
    for provider in requested:
        if provider not in ONLINE_PROVIDER_CATALOG and provider!="ollama":
            errors.append({"provider":provider,"phase":"preflight","error":"UNKNOWN_REVIEW_PROVIDER"});continue
        if provider!="ollama" and not _provider_credentials(provider):
            errors.append({"provider":provider,"phase":"preflight","error":"PROVIDER_NOT_CONFIGURED"});continue
        preflight=_provider_runtime_preflight(provider,"review",True)
        if preflight.get("ok"):
            ready.append(provider)
        else:
            errors.append({"provider":provider,"phase":"preflight","error":clean_inline(str(preflight.get("detail") or "Provider is not ready"))[:700]})
    # Gold review should not fail simply because selected online reviewers are unavailable.
    # Add a healthy local reviewer when fewer than two selected reviewers are actually ready.
    if len(ready)<2 and "ollama" not in ready and _provider_credentials("ollama"):
        preflight=_provider_runtime_preflight("ollama","review",True)
        if preflight.get("ok"):
            ready.append("ollama")
        else:
            errors.append({"provider":"ollama","phase":"fallback_preflight","error":clean_inline(str(preflight.get("detail") or "Local reviewer is not ready"))[:700]})
    return ready,errors
...
def _multi_review_worker(job_id,payload):
    try:
        ...
        requested_source=payload.get("reviewers") if "reviewers" in payload else settings().get("reviewer_providers")
        requested=[str(x).lower() for x in (requested_source or [])]
        requested=[x for x in requested if x in ONLINE_PROVIDER_CATALOG or x=="ollama"][:10]
        ready,preflight_errors=_review_ready_pool(requested)
        context=payload.get("context") if isinstance(payload.get("context"),dict) else {}
        if not ready:
            _review_update(job_id,state="unavailable",progress=100,stage="Gold review unavailable",reviewers=[],reviews=[],errors=preflight_errors,error="NO_READY_REVIEW_PROVIDER")
            return
        _review_update(job_id,state="working",progress=8,stage="Ready reviewers selected",reviewers=ready,requested_reviewers=requested,reviews=[],errors=preflight_errors)
        results=[];errors=list(preflight_errors);done=0
        with ThreadPoolExecutor(max_workers=min(5,len(ready))) as ex:
            futs={ex.submit(_review_one_provider,p,draft,title,context):p for p in ready}
            for fut in as_completed(futs):
                p=futs[fut]
                try:results.append(fut.result())
                except Exception as e:errors.append({"provider":p,"phase":"review","error":clean_inline(str(e))[:700]})
                done+=1;_review_update(job_id,progress=10+int(65*done/max(1,len(ready))),stage=f"Reviewer {done}/{len(ready)} completed",reviews=results,errors=errors)
        # A provider can pass a credential/status endpoint but still fail at generation (for example,
        # an account with no credits). If every ready online reviewer failed, try healthy Ollama once.
        if not results and "ollama" not in ready and _provider_credentials("ollama"):
            preflight=_provider_runtime_preflight("ollama","review",True)
            if preflight.get("ok"):
                _review_update(job_id,progress=76,stage="Online reviewers unavailable; trying local reviewer",reviewers=ready+["ollama"],errors=errors)
                try:results.append(_review_one_provider("ollama",draft,title,context))
                except Exception as e:errors.append({"provider":"ollama","phase":"fallback_review","error":clean_inline(str(e))[:700]})
            else:errors.append({"provider":"ollama","phase":"fallback_preflight","error":clean_inline(str(preflight.get("detail") or "Local reviewer is not ready"))[:700]})
        if not results:
            _review_update(job_id,state="unavailable",progress=100,stage="Gold review unavailable",reviews=[],errors=errors,error="ALL_REVIEW_MODELS_UNAVAILABLE")
            return
        ...
        if lead=="auto" or lead not in successful:
            lead=max(successful,key=lambda p:ONLINE_PROVIDER_CATALOG.get(p,{}).get("review_score",7.0 if p=="ollama" else 0))
        ...
        try:
            ...
            errors.append({"provider":lead,"phase":"lead_review","error":clean_inline(str(e))[:700]})
            _review_update(job_id,state="unavailable",progress=100,stage="Lead review unavailable",reviews=results,errors=errors,error="LEAD_REVIEW_UNAVAILABLE",reviewer_count=len(results))
            return
...
def generate_draft(ctx,plan,research,requested_model="auto",progress=None):
    ...
    chunks=_draft_chunks(sections,4)
    for chunk_index,chunk in enumerate(chunks,1):
        if callable(progress):
            progress(32 + (chunk_index-1)*max(1,28/max(1,len(chunks))), f"AI drafting section group {chunk_index}/{len(chunks)}")
...
def run_specialist_agents(ctx,plan,research,requested_model="auto",progress=None):
    reports=[];models=[];evidence=_research_for_prompt(research,15000);uploads=_uploaded_documents_for_prompt(ctx,12000)
    team=list(plan.get("agent_team") or [])
    for idx,agent in enumerate(team,1):
        if callable(progress):
            progress(32 + (idx-1)*max(1,18/max(1,len(team))), f"Specialist AI {idx}/{len(team)} drafting assigned sections")
...
def lead_assemble_multi_agent_document(ctx,plan,research,reports,requested_model="auto",progress=None):
    ...
    for chunk_index,chunk in enumerate(chunks,1):
        if callable(progress):
            progress(52 + (chunk_index-1)*max(1,12/max(1,len(chunks))), f"Lead Editor assembling section group {chunk_index}/{len(chunks)}")
...
def generate_project_draft(ctx,plan,research,requested_model="auto",progress=None):
    if plan.get("agent_mode")!="multi":
        draft,models=generate_draft(ctx,plan,research,requested_model,progress)
    ...
    reports,specialist_models=run_specialist_agents(ctx,plan,research,requested_model,progress)
    draft,lead_models=lead_assemble_multi_agent_document(ctx,plan,research,reports,requested_model,progress)
...
def agent_create_document(d, progress=None):
    def report(percent, stage):
        if callable(progress):
            try:progress(float(percent),str(stage))
            except Exception:pass
    reset_ai_run_state()
    report(4,"Reading project brief")
    ...
    report(10,"Planning document structure")
    ...
    report(18,"Gathering research and source context")
    research=build_research_package(plan,ctx)
    report(30,"AI drafting started")
    draft,model_trace,agent_run=generate_project_draft(ctx,plan,research,route_request,report)
    report(68,"AI draft complete; validating sections")
    draft,audit,repair_models=repair_draft(ctx,plan,research,draft,route_request,2)
    validation_warning = None
    if audit.get("ok"):
        report(82,"Validation and repair complete")
    elif _r316_short_only_warning(audit):
        validation_warning = {
            "code": "SECTIONS_REMAIN_SHORT_AFTER_TARGETED_REPAIR",
            "short": list(audit.get("short") or []),
            "message": "The document was generated and all required sections are present, but some sections remain shorter than the preferred quality threshold.",
        }
    ...
    report(98,"Saving document history")
    ...
    report(99,"Final checks")
    return {"ok":True,"job_id":job_id,"project_name":title,"plan":plan,"agent_run":agent_run,"research":research,"draft":draft,"audit":audit,"validation_warning":validation_warning,"model_trace":model_trace+repair_models,"ai_router":ai_status(),"last_route_attempts":AI_LAST_ATTEMPTS,"files":files,"job_folder":str(job_dir)}


# AGAPE R31.16 ASYNC DOCUMENT CREATION JOBS
# Long document creation can legitimately take longer than an HTTP socket should remain open.
# Keep the original synchronous endpoint for compatibility, but expose a background job API
# so Mainframe can poll progress without turning a healthy long AI run into a transport timeout.
AGENT_CREATE_JOBS={}
AGENT_CREATE_LOCK=threading.Lock()

def _agent_create_job_update(job_id,**kw):
    with AGENT_CREATE_LOCK:
        row=AGENT_CREATE_JOBS.setdefault(str(job_id),{})
        # Progress is monotonic. Heartbeats and nested phases can never move the bar backwards.
        if 'progress' in kw:
            try:kw['progress']=max(float(row.get('progress') or 0),float(kw.get('progress') or 0))
            except Exception:kw['progress']=row.get('progress') or kw.get('progress') or 0
        row.update(kw);row['updated_at']=time.time()
        return dict(row)

def _agent_progress_cap(stage):
    text=str(stage or '').lower()
    if 'reading' in text:return 8
    if 'planning' in text:return 16
    if 'research' in text:return 29
    if 'specialist' in text:return 51
    if 'lead editor' in text:return 66
    if 'draft' in text or 'ai' in text:return 66
    if 'validat' in text or 'repair' in text:return 81
    if 'building' in text:return 93
    if 'pdf' in text:return 97
    if 'saving' in text or 'final' in text:return 99
    return 64

def _agent_create_job_heartbeat(job_id,stop_event):
    started=time.time()
    while not stop_event.wait(5.0):
        with AGENT_CREATE_LOCK:
            row=dict(AGENT_CREATE_JOBS.get(str(job_id),{}))
        if str(row.get('state') or '').lower()!='working':return
        stage=str(row.get('stage') or 'Creating document with AI')
        current=float(row.get('progress') or 1)
        cap=float(_agent_progress_cap(stage))
        # Make visible progress inside a long provider call, but never claim the phase is complete.
        if current < cap:
            # Approach the phase ceiling gradually instead of reaching it early and
            # appearing frozen during a genuinely long provider call.
            gap=max(0.0,cap-current)
            step=max(0.03,min(0.35,gap*0.035))
            current=min(cap-0.01 if cap<100 else cap,current+step)
        elapsed=max(1,int(time.time()-started))
        _agent_create_job_update(job_id,progress=current,message=f"{stage} · still working · {elapsed}s",heartbeat_at=time.time())

def _agent_create_job_worker(job_id,payload):
...
def start_agent_create_job(payload):
    job_id='DOC-'+hashlib.sha256((str(time.time_ns())+os.urandom(8).hex()).encode()).hexdigest()[:20]
    _agent_create_job_update(job_id,state='working',progress=1,stage='Queued',started_at=time.time())
    threading.Thread(target=_agent_create_job_worker,args=(job_id,dict(payload or {})),daemon=True).start()
    return {'ok':True,'job_id':job_id}

def agent_create_job_status(job_id):
    with AGENT_CREATE_LOCK:
        row=dict(AGENT_CREATE_JOBS.get(str(job_id or ''),{}))
    if not row:return {'ok':False,'error':'AGENT_CREATE_JOB_NOT_FOUND'}
    row['ok']=True
    return row
# AGAPE R31.16 ASYNC DOCUMENT CREATION JOBS END
...
HTML=r"""<!doctype html><html><head><meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate"><meta http-equiv="Pragma" content="no-cache"><meta http-equiv="Expires" content="0"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Agape Document Studio R31.16</title><style>
...
</style></head><body><header><h1>Agape Document Studio R31.16</h1><p>OpenDocument first - LibreOffice powered - verified templates - persistent document history.</p></header><div id="workProgressWrap" class="work-progress"><div class="work-progress-box"><div class="work-track"><div id="workProgressFill" class="work-fill"></div></div><div id="workProgressLabel" class="work-label">Ready.</div></div></div><div class="wrap">
...
async function refreshSources(){try{log('Testing source links...');let j=await api('/api/sources/refresh',{method:'POST'});renderSources(j.sources);log('Link testing complete.')}catch(e){log(e)}}async function installExternal(){try{log('Downloading and validating open templates...');let j=await api('/api/templates/install-open',{method:'POST'});log(j);await refresh()}catch(e){log(e)}}async function saveSettings(){try{let body={country_of_origin:$('country').value,locale:$('locale').value,paper_size:$('paper').value,preferred_engine:'LibreOffice',ai_provider:$('aiProvider').value,ai_model:$('aiModel').value,research_enabled:$('researchEnabled').checked,research_depth:$('researchDepth').value,ingestion_engine:$('ingestionEngine').value,rag_enabled:$('ragEnabled').checked,also_pdf:$('alsoPdf').checked,open_after:$('openAfter').checked,template_mode:$('templateMode').value,review_after_create:$('reviewAfterCreate').checked,reviewer_providers:selectedReviewers(),lead_reviewer:$('leadReviewer').value,provider_auth_modes:selectedProviderAuthModes()};let j=await api('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});log(j);await refresh()}catch(e){log(e)}}async function openLibreOffice(){log(await api('/api/libreoffice/open',{method:'POST'}))}async function openFolder(){log(await api('/api/folder/open',{method:'POST'}))}async function initialiseDocumentStudio(){
 try{
  ...
  validateMissingInfo();
  console.log('AGAPE_FORM_INIT=PASS VERSION=R31.16');
 }catch(e){
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=="/api/agent-create/status":
            q=urllib.parse.parse_qs(u.query);return json_response(self,200,agent_create_job_status((q.get("id") or [""])[0]))
    ...
    def do_POST(self):
        ...
        try:
            ...
            if u.path=="/api/agent-create/start":
                d=self.body()
                return json_response(self,202,start_agent_create_job(d))
...
def _r316_section_body(text, heading):
    ...
    best = ""
    for match in _r310_markdown_blocks(text):
        if _r310_heading_key(_r310_display_text(match.group(2))) == target:
...
def _r316_repair_target_chars(doc_type, heading):
    minimum = int(_r310_min_section_chars(doc_type, heading))
    key = _r310_heading_key(heading)
    if str(doc_type or "") in ("Business Proposal", "Business Report", "Technical Report"):
        if key in {"findings", "analysis"}:
            return max(420, minimum * 3)
        return max(300, minimum * 2)
    return max(220, minimum * 2)
...
def repair_draft(ctx, plan, research, draft, requested_model="auto", max_rounds=2):
    ...
    for round_index in range(max(1, int(max_rounds))):
        ...
        # Repair one section at a time. This avoids a small/slow model spending its
        # whole response budget on the first heading and leaving later headings thin.
        for heading in needs:
            minimum = int(_r310_min_section_chars(doc_type, heading))
            target = _r316_repair_target_chars(doc_type, heading)
            system = (
                "You repair one professional-document section at a time. Return only the exact requested # heading "
                "and its replacement body. The body must be substantive and decision-useful. Use supplied facts and "
                "research only; label limitations or assumptions and never invent private facts or unsupported figures."
            )
            prior_len = len(_r316_section_body(draft, heading))
            patch = ""
            for strict_attempt in range(2):
                prompt = (
                    "FORM_STATE:\n" + json.dumps(_ai_context_snapshot(ctx, 0, 32, False), ensure_ascii=False)
                    + "\nPLAN:\n" + json.dumps(plan, ensure_ascii=False)
                    + "\nRESEARCH:\n" + _research_for_prompt(research, 9000)
                    + "\nCURRENT_DOCUMENT:\n" + draft[-14000:]
                    + "\n\nTARGET SECTION: # " + str(heading)
                    + "\nVALIDATOR_MINIMUM_BODY_CHARACTERS: " + str(minimum)
                    + "\nREPAIR_TARGET_BODY_CHARACTERS: at least " + str(target)
                    + "\nCURRENT_BODY_CHARACTERS: " + str(prior_len)
                    + "\nWrite 2-5 useful paragraphs or structured bullets where appropriate. "
                      "Do not return placeholders. Do not return any other document section."
                )
                if strict_attempt:
                    prompt += (
                        "\n\nSTRICT RETRY: the prior repair response was still too short or used the wrong heading. "
                        "Use the exact heading and make the body clearly longer than the repair target."
                    )
                r = ai_generate(prompt, system, max_tokens=2200, requested_model=requested_model, timeout=240, task="repair")
                candidate = str(r.get("content") or "").strip()
                candidate_len = len(_r316_section_body(candidate, heading))
                models.append({
                    "provider": r.get("provider"), "model": r.get("model"),
                    "round": round_index + 1, "section": heading,
                    "strict_attempt": strict_attempt + 1, "body_chars": candidate_len,
                })
                patch = candidate
                if candidate_len >= minimum:
                    break
            if patch:
...
def serve(port,open_browser=True):
    generate_templates();db();srv=ThreadingHTTPServer(("127.0.0.1",port),Handler);print("AGAPE_DOCUMENT_STUDIO_R31_11=READY",flush=True)
