...
const (
	buildName              = "AGAPE-WINDOWS11-MAIN-PROJECT-R7.1"
...
var activeInstallRoot string

func programsRoot() string      { return filepath.Join(os.Getenv("LOCALAPPDATA"), "Programs") }
func legacyInstallRoot() string { return filepath.Join(programsRoot(), "Agape") }
func installRoot() string {
	if activeInstallRoot != "" {
		return activeInstallRoot
	}
	if exe, err := os.Executable(); err == nil && strings.EqualFold(filepath.Base(exe), "Agape-Uninstall.exe") {
		return filepath.Dir(exe)
	}
	return filepath.Join(programsRoot(), "Agape-Windows11-R7.1")
}
...
func localUIPreflight(core string) error {
	...
	s := string(b)
	markers := []string{"data-page=\"chat\"", "data-page=\"settings\"", "workspace-manage-projects", "project-instructions-text", "project-document-choose", "Choose Document", "auto-choose-model", "/api/models/recommend", "/api/connections"}
	n := 0
...
func createShortcutsFor(rootPath string) error {
	exePath := filepath.Join(rootPath, "Agape.exe")
...
func createShortcuts() error { return createShortcutsFor(installRoot()) }
...
func registerUninstall() error {
	for _, v := range [][3]string{{"DisplayName", "Agape", "REG_SZ"}, {"DisplayVersion", "3.1-Windows11-R7.1", "REG_SZ"}, {"Publisher", "DMT", "REG_SZ"}, {"InstallLocation", installRoot(), "REG_SZ"}, {"DisplayIcon", appPath() + ",0", "REG_SZ"}, {"UninstallString", "\"" + uninstallerPath() + "\" --uninstall", "REG_SZ"}, {"NoModify", "1", "REG_DWORD"}, {"NoRepair", "1", "REG_DWORD"}} {
...
func commitCandidate(candidate, target string) error {
	if _, e := os.Stat(target); e == nil {
		return fmt.Errorf("NEW_NATIVE_TARGET_ALREADY_EXISTS=%s", target)
	}
	if e := os.Rename(candidate, target); e != nil {
		return fmt.Errorf("CANDIDATE_COMMIT_FAILED=%w", e)
	}
	return nil
}
func restoreCutover(newRoot, registryBackup, previousRoot string) {
	removeShortcuts()
	_ = os.RemoveAll(newRoot)
	restoreOldRegistry(registryBackup)
	if previousRoot != "" {
		if st, e := os.Stat(filepath.Join(previousRoot, "Agape.exe")); e == nil && !st.IsDir() {
			_ = createShortcutsFor(previousRoot)
		}
	}
}
...
func install() error {
	fmt.Println("============================================================")
	fmt.Println(" AGAPE WINDOWS 11 MAIN PROJECT NATIVE DESKTOP R7.1")
	...
	target := filepath.Join(programsRoot(), "Agape-Windows11-R7.1-"+stamp)
	activeInstallRoot = target
	previousRoot := legacyInstallRoot()
	if st, x := os.Stat(previousRoot); x == nil && st.IsDir() {
		say("PREVIOUS_DESKTOP_PRESERVED", previousRoot)
	}
	candidate := filepath.Join(programsRoot(), "Agape-Windows11-R7.1-Candidate-"+stamp)
	...
	if e = commitCandidate(candidate, target); e != nil {
		return e
	}
	say("SIDE_BY_SIDE_CUTOVER", "PASS_NO_LIVE_FOLDER_RENAME")
	say("NEW_NATIVE_INSTALL", target)
	if e = copySelf(uninstallerPath()); e != nil {
		restoreCutover(target, registryBackup, previousRoot)
