...
PROJECT_TEMPLATE_DIR = DATA_ROOT / "project-templates"

def _project_template_path(project_id: int) -> Path:
    return PROJECT_TEMPLATE_DIR / ("project-" + str(int(project_id)) + ".json")

def _validate_project_template(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict): raise ValueError("TEMPLATE_ROOT_MUST_BE_OBJECT")
    version=str(value.get("template_version") or "").strip()
    if not version.startswith("DMT-PROJECT-TEMPLATE-"): raise ValueError("UNSUPPORTED_TEMPLATE_VERSION")
    if not str(value.get("template_name") or "").strip(): raise ValueError("TEMPLATE_NAME_REQUIRED")
    if not isinstance(value.get("project_state"), dict): raise ValueError("PROJECT_STATE_OBJECT_REQUIRED")
    if not str(value.get("system_instruction") or "").strip(): raise ValueError("SYSTEM_INSTRUCTION_REQUIRED")
    return value

def _save_project_template(project_id: int, value: Any) -> dict[str, Any]:
    if not db.project(project_id): raise ValueError("PROJECT_NOT_FOUND")
    value=_validate_project_template(value)
...
def _load_project_template(project_id: int) -> dict[str, Any] | None:
    try: return _validate_project_template(json.loads(_project_template_path(project_id).read_text(encoding='utf-8')))
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/api/startup/status":
                ...
                return
            if path == "/api/project-template/status":
                raw=str((query.get("project_id") or [""])[0]).strip()
                if not raw: raise ValueError("PROJECT_ID_REQUIRED")
                pid=int(raw); value=_load_project_template(pid)
                self.send_json({"ok":True,"active":bool(value),"template":value,"project_id":pid})
                return
            if path == "/api/models/recommend":
                task = str((query.get("task") or [""])[0])
                raw_project = str((query.get("project_id") or [""])[0]).strip()
                try:
                ...
                if raw_project:
                    ...
                    if not item: raise ValueError("PROJECT_NOT_FOUND")
                    settings = db.project_loop_settings(project_id) or {}
                    workspace = str(settings.get("workspace") or "").strip()
                    index = codebase_index.build_index(workspace, 1500) if workspace else {"ok":False,"error":"WORKSPACE_NOT_CONFIGURED","files":[]}
                    result = model_router.choose_project_model(
                        installed, task, item, db.list_messages(project_id, 80), settings,
                        db.project_loop_runs(project_id), db.list_issues(project_id), index
                    )
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/releases/checkpoint":
                ...
                return
            if path == "/api/project-template/import-document":
                name=str(data.get("name") or "document").strip()
                encoded=str(data.get("content_base64") or "").strip()
                if not encoded: raise ValueError("DOCUMENT_CONTENT_REQUIRED")
                try: raw=base64.b64decode(encoded,validate=True)
                except Exception as exc: raise ValueError("DOCUMENT_BASE64_INVALID") from exc
                if len(raw)>5*1024*1024: raise ValueError("DOCUMENT_TOO_LARGE_MAX_5MB")
                suffix=Path(name).suffix.lower()
                if suffix in {".txt",".md",".json",".csv",".log",".ini",".yaml",".yml",".toml",".html",".htm",".xml",".py",".js",".ts",".css",".c",".h",".cpp",".hpp",".asm",".s",".ps1"}:
                    ...
                    try:
                        ...
                        paras=[]
                        for pnode in root.iter(ns+"p"):
                            parts=[t.text or "" for t in pnode.iter(ns+"t")]
                            line="".join(parts).strip()
                            if line: paras.append(line)
                        text="\n\n".join(paras)
                    except Exception as exc: raise ValueError("DOCX_TEXT_EXTRACTION_FAILED") from exc
                else:
                    raise ValueError("DOCUMENT_TYPE_NOT_SUPPORTED")
                text=text.strip()
                if not text: raise ValueError("DOCUMENT_HAS_NO_TEXT")
                if len(text)>200000: raise ValueError("DOCUMENT_TEXT_TOO_LARGE_MAX_200000_CHARS")
                self.send_json({"ok":True,"name":name,"text":text,"characters":len(text)})
                return
            if path == "/api/project-template/activate":
                pid=int(data.get("project_id") or 0)
                value=dict(data.get("template") or {})
                saved=_save_project_template(pid,value)
                state=saved.get("project_state") or {}
                current=db.project_loop_settings(pid) or {}
                workspace=str(state.get("authoritativeFile") or current.get("workspace") or "").strip()
                if workspace and Path(workspace).suffix: workspace=str(Path(workspace).parent)
                goal=str(state.get("goal") or current.get("goal") or "").strip()
                if workspace and goal:
                    configure_project_loop(project_id=pid,workspace=workspace,goal=goal,test_command=str(current.get("test_command") or ""),max_steps=int(current.get("max_steps") or 4),auto_model=True,model=str(current.get("model") or ""))
                self.send_json({"ok":True,"active":True,"template":saved,"project_id":pid})
                return
            if path == "/api/project-template/clear":
                pid=int(data.get("project_id") or 0); path2=_project_template_path(pid)
                if path2.exists(): path2.unlink()
                self.send_json({"ok":True,"active":False,"project_id":pid})
                return
